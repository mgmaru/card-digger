...
> **追記（2026-09-01）。** この検証は`created`の逆転だけを数えていた。
> 2026-09-01に`updated`でも測ったところ、**`updated`には降順の傾向がある**（破れ21%）。
> また`ORDER_ASC`と`ORDER_DESC`で**返る順序が完全に一致**しており、
> **`order`パラメータは結果に影響していない**ことが分かった。
> 詳細は[`created`と`updated`の追加観測結果](timestamp-result.md)。
> **「古い順にならない」という本検証の結論は変わらない。**
...
- `num_sell_items`取得成功: **10 / 10（100%）**
- 追加リクエスト数: **10**
- Profile取得時間 中央値 / 最大値: **72.22ms / 108.07ms**
- 評価件数: **0〜8,624件、中央値360件**
- `num_sell_items`: **2〜8,704件、中央値302.5件**（値の範囲だけを観測。**意味は未確認**）
- 取得を実測した項目: 名前、評価内訳、評価件数、星評価、Score、`num_sell_items`
- 取得できなかった必須項目: **なし**

> **訂正（2026-09-01）。** この節は当初`num_sell_items`を「販売件数」と記載していた。
> 値は取得できていたが、**何を数えているかは確認していなかった。**
> 2026-09-01の[追加観測](open-questions-result.md)で**出品件数であり販売件数ではない**と判明した。
> この1行がAdapterの型名（`total_sales_count`）とMVPの表示要件（「累計販売件数」）へ伝わっている。
> **取得率の実測値は変わらない。** 経緯は[検証の落とし穴](../../docs/retrospectives/2026-09-01-verification-pitfalls.md)。
