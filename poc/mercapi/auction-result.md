...
> **判定規則についての追記（2026-09-01）。** 本検証の「一致」は厳密比較ではなく、
> **ページ本文から拾った¥金額10件のどれかにAPI値が含まれるか**という包含判定だった。
> どの金額が現在価格かを特定していない。
>
> 原因は、ページ本文に`現在の価格`というラベルが20 / 20で存在せず、狙い撃ちできないと
> 判断したことにある。**DOMを調べていなかった。** 実際には価格要素に`data-testid="price"`が
> 付いており、テキストは`現在 ¥900`（ラベルは`現在`）だった。
>
> `auction_probe.py`は2026-09-01に価格要素との1対1比較へ変更した。
> **本結果の合否は変わらない**（基準は95%以上一致）が、この10 / 10を
> 「Adapterの価格選択が正しいことの証拠」として読んではならない。詳細は
> [ライブ受入検証結果 §12.1](../../docs/phase-0/phase-0-f-live-acceptance-result.md#121-何を変えどの弱点が解消したか)。
