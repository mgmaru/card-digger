...
- [ ] 31件以上の商品が存在するSellerを検証対象として選定する
- [ ] Sellerページの初回表示時に発生する商品一覧API通信を記録する
- [ ] スクロールまたは追加読み込み時に発生するAPI通信を記録する
- [ ] 商品一覧APIのEndpoint、HTTP Method、`limit`、販売状態指定を確認する
- [ ] `cursor`、`offset`、`pageToken`など、続きを取得するRequest Parameterを確認する
- [ ] Responseに含まれる次ページ情報と終端判定情報を確認する
- [ ] Playwrightで31件目以降の商品を取得し、商品IDの重複と販売状態を確認する
- [ ] 販売中商品と売却済み商品をそれぞれページングできるか確認する
- [ ] `mercapi`の30件上限がMercari API自体の制限か、ライブラリの実装不足かを判定する
- [ ] 確認したRequest / Responseの概要、取得件数、失敗内容、未確定事項を検証結果に記録する
