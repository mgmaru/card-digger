...
**ただし相場データそのものはCard Diggerが持たない**（[§19](#19-相場データはcard-diggerが持たない2026-09-05決定)）。比較はするが、値を集めるのは[card-pulse](https://github.com/mgmaru/card-pulse)の仕事である。
...
- 評価
- 評価件数
- Profile上の出品件数（`num_sell_items`。累計販売件数ではない）
- 取得した販売中商品数（最大100件）
- 取得した売却済み商品数（最大100件）
...
- キーワード検索
- 価格フィルター
- 取得範囲内の掲載開始日・終了日フィルター
- 通常出品・オークション・形式不明の表示とフィルター
- 取得範囲内の古い順
- 新しい順
- 商品画像のグリッド表示
- 元Mercari商品ページへのリンク
- 取得範囲・最古・最新日時・取得時刻・打ち切り理由の表示
- オークション価格が取得時点の値であることの表示
...
**カード相場データはcard-pulseから受け取る**（[§19](#19-相場データはcard-diggerが持たない2026-09-05決定)）。
Card Diggerが集めるものではない。
...
# 14. 最初に作成するGitHub Issues案 — **Phase 0で消化済み**

**着手前に18件のIssue案をここへ並べていた。内容はPhase 0ですべて片付いたため削除した。**
未完了の見た目のまま残しておくと、終わった作業がこの文書を読むたびに未着手に見える。

結果は次にある。この文書へ複製しない。

| 何を知りたいか | 正本 |
|---|---|
| 3方式の比較と採用方式の決定 | [Phase 0-E 取得方式の選定](../phase-0/phase-0-e-selection.md) |
| 各方式を同じ条件で測った手順と結果 | [共通検証プロトコル](../phase-0/poc-validation.md) |
| `created`・ページング・Seller情報など個別の観測 | [PoC検証結果](../phase-0/poc-validation.md) |
| 実Mercariへ接続した受入検証 | [ライブ受入検証の結果](../phase-0/phase-0-f-live-acceptance-result.md) |

**進行中の作業は[TODO](../planning/todo.md)が唯一の置き場所である。**
...
## PoC成功 — **2026-09-01に全項目達成**

以下が確認できればPhase 1へ進む、として置いた条件である。**Phase 0-E / 0-Fで全項目を満たし、
Phase 1へ進んでいる。**
...
達成の根拠は[Phase 0-E 取得方式の選定](../phase-0/phase-0-e-selection.md)と
[ライブ受入検証の結果](../phase-0/phase-0-f-live-acceptance-result.md)にある。
**checkboxを外したのは、この節が条件の定義であって進捗ではないためである。**
進捗は[TODO](../planning/todo.md)が持つ。
...
# 19. 相場データはCard Diggerが持たない（2026-09-05決定）

**「買う候補を探す」のがCard Digger、「どの程度の価値を持つか」を答えるのが
[card-pulse](https://github.com/mgmaru/card-pulse)である。** 別Repositoryの独立したデータ基盤で、
役割分担の出所はcard-pulse側の
[連携構想](https://github.com/mgmaru/card-pulse/blob/main/concepts/card_digger_market_data_architecture_concept.md)。

| | 引き受けるもの |
|---|---|
| **Card Digger** | フリマサイトから**買う候補を探す**。検索、画像での一次選別、Seller評価、探索空間を縮めること |
| **card-pulse** | カードが**どの程度の価値を持つか**。買取価格、店舗差、ショップ販売価格、**フリマの出品価格と成約価格**、価格推移、買取店舗数 |

## Card Diggerが計算しないもの

card-pulseが自分の扱うデータとして挙げているものは、Card Digger側で作らない。
**同じ判断が2か所にあると、どちらが正しいかを決める仕事が新しく生まれる。**

- カードごとの相場
- **売却済み商品の価格分布**（Mercariの成約価格。2026-09-05にこの理由で見送った。
  経緯は[TODO](../planning/todo.md#売却済み商品の価格分布は作らない--card-pulseの担当2026-09-05決定)）
- 推定総額、想定利益、手数料の考慮

**Card Diggerは受け取って表示する側になる。** [Phase 3](#phase-3--market-value)の
「カード相場データ」はcard-pulseから来るものであって、Card Diggerが集めるものではない。

## 連携の形はまだ決めていない

**API、スキーマ、どちらがどちらを呼ぶかは未定。** card-pulseは構想・調査文書だけの段階である。

## 答えの無い問いが1つ残っている

card-pulse MVPの範囲は**買取価格・1 TCG・構造化Web情報源2〜3店舗**で、
**フリマの成約価格はそこに入っていない。** 「向こうがやる」と書いて終わりにできないので、
残っていること自体をここに書く。

> **このまとめ売り、5,000円は高いのか安いのか。**

**今はどちらも答えない。** 答えるには[Phase 2](#phase-2--listing-analysis)（写真から枚数と種類）と
card-pulseの両方が要る。**片方だけでは答えにならない。**

---

# 20. 「効率的か」は利用者にしか答えられない（2026-09-05決定）

**Phaseを閉じるとき、必ずこの問いを立てる。**

> **人間が見る商品を効率的に絞れるようになったか。**

[§18](#18-将来像)が「探索空間を縮めること」をこの製品の中心的な価値としている以上、
**それが起きたかどうかがPhaseの合否である。** Testが緑でも、依存が増えていなくても、
探索が速くなっていなければそのPhaseは目的を果たしていない。

## 機械は答えられない

Testも型検査もCIも、この問いに触れられない。**使った本人だけが答えられる。**
だから完了条件の他の項目と違い、**開発者側で`[x]`を入れてはならない。**

**Phaseを機械的な緑だけで閉じない。** 完了条件の残り全部が緑でも、この1件は別に問う。

## 「はい」で終わらせない

**答えと一緒に、まだ遅かったことを1行残す。** ここがこの問いの実質である。

「効率的である」だけでは次に何をすべきかが残らない。**この製品の追加作業は、
すべてこの気づきから出てきた。**

| 気づき | そこから出た作業 |
|---|---|
| 商品の状態が分からない / 更新日で絞れない | [使ってみて分かった2件](../planning/todo.md#使ってみて分かった2件2026-09-04)（2026-09-04に実装） |
| 出品者が生きているか分からない | [`is_inactive`をSeller画面へ](../planning/todo.md#出品者が生きているかを直接知る2026-09-05に実装)（2026-09-05に実装） |
| **まとめ売りの値段が高いか安いか分からない** | [A](../planning/todo.md#売却済み商品の価格分布は作らない--card-pulseの担当2026-09-05決定)（検討の結果、[§19](#19-相場データはcard-diggerが持たない2026-09-05決定)でcard-pulseの担当と決めた） |

**「何も遅くなかった」という答えを疑う。** 使っていれば必ずどこかで止まる。
止まらなかったのなら、まだ十分に使っていない。

## Phase 1の答え（2026-09-05）

**「効率的である」。** [MVP完了条件](mvp-spec.md#mvp完了条件)の10件目に`[x]`が入った。
同時に「使ってみてより効率的にしていく」という方針が示され、この節はそこから生まれた。
