...
- 更新日Filter（**◯日以上更新されていない商品だけを残す。取得済みの集合に対して行う**）
...
| 未更新日数 | 未指定、または0以上の整数。単位は日。`updatedAt`から`collectedAt`までの日数がこの値以上の商品を残す |
...
- 未更新日数: `updatedAt`から`collectedAt`までの日数 `>=` 指定値を残す（[下記](#更新日filter2026-09-04決定)）
...
**選択肢を並べない理由。** 出所のある日数は棒の頭打ちと同じ365日だけで、
30 / 90 / 180には根拠が無い（[§5.6](#未更新期間の棒2026-09-02決定)、[§5.3](#53-収集範囲)）。
入力欄のPlaceholderに`365`を置き、選ぶのは利用者に任せる。

**これで奥へ遡れるようにはならない。** 取得済みから取り除くだけで、取ってこなかった商品は
増えない（[§5.3](#価格帯だけが到達範囲を変える2026-09-03)）。到達範囲を変えるのは価格帯だけである。

**Sellerが活動しているかは、これでは分からない。** 1年触られていない商品は、その出品者が
まだいるかどうかを何も語らない。それに答えるのは[§6.2の「最も新しい更新」](#最も新しい更新を出す2026-09-03決定)で、
**開いた1人についてだけ**である。検索結果全体へ広げない理由は
[TODO O-8](../planning/todo.md#o-8--sellerの活動で検索結果を絞るのを打ち切った理由2026-09-04)。
