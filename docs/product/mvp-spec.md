...
| 未更新日数（下限） | 未指定、または0以上の整数。単位は日。`updatedAt`から`collectedAt`までの日数がこの値**以上**の商品を残す |
| 未更新日数（上限） | 未指定、または0以上の整数。同じ日数がこの値**以下**の商品を残す |
| 未更新日数の範囲 | 両方指定時は下限 `<=` 上限 |
...
掲載日・未更新日数・販売形式のFilterとSortだけがFrontendの計算で、取得済み範囲へ適用し
再Requestしない。
...
- 未更新日数: `updatedAt`から`collectedAt`までの日数が、下限`以上`かつ上限`以下`の商品を残す。**両端とも境界を含む**（[下記](#更新日filter2026-09-04決定)）
- 商品の状態: 選んだ状態と、**それより良い状態**の商品を残す。`状態不明`は残す（[下記](#状態filter2026-09-05決定)）
...
| 項目 | 決めたこと |
|---|---|
| 軸 | **商品の`updatedAt`。** Sellerの活動ではない（下記） |
| 数え方 | `updatedAt`から**`collectedAt`まで**の日数。[§5.6](#未更新期間の棒2026-09-02決定)の棒と同じ |
| 入力 | 0以上の整数。両端とも**境界を含む**。**選択肢を並べない** |
| 未指定 | その端では絞らない |
| 下限 > 上限 | 上限を無効にし、対象Fieldの近くに理由を出す（掲載日と同じ扱い） |

#### 両端が要る理由 ―― 逆の問いに答えている

**片方はもう一方の既定ではない。** 同じ軸だが、答えている問いが違う。

| | 残るもの | 何のため |
|---|---|---|
| **◯日以上** | **放置されている出品** | この製品が探しているもの。[`updated_asc`](#55-sortとfilter)と[未更新期間の棒](#未更新期間の棒2026-09-02決定)が向いている先と同じ |
| **◯日以下** | **最近動いた出品** | 買えるうちに買うため。動きが無い出品を追いかける時間を減らす |

**「アクティブな出品者かつ更新が古い商品」は、この軸1本では書けない。** 1つの商品の
`updatedAt`は「最近」と「古い」を同時に満たせないためである。片方は出品者について、
もう片方はその商品についての条件であり、対象が違う。

#### 選択肢を並べない

出所のある日数は棒の頭打ちと同じ365日だけで、30 / 90 / 180には根拠が無い
（[§5.6](#未更新期間の棒2026-09-02決定)、[§5.3](#53-収集範囲)）。
**Placeholderを置くのも下限だけ**とし、選ぶのは利用者に任せる。

#### これで奥へ遡れるようにはならない

取得済みから取り除くだけで、取ってこなかった商品は増えない
（[§5.3](#価格帯だけが到達範囲を変える2026-09-03)）。到達範囲を変えるのは価格帯だけである。

#### Sellerが活動しているかは、これでは分からない

**`◯日以下`で残るのは「最近触られた出品」であって「アクティブな出品者」ではない。**
Mercariは`updatedAt`にラベルを付けておらず、**出品者本人が動かしたのかを確認していない**
（[§5.5](#updatedatの表示ラベル)）。だから画面の語も`未更新日数`であり、`アクティブ`ではない。
[§6.2](#最も新しい更新を出す2026-09-03決定)が「最終活動」ではなく「最も新しい更新」と書くのと
同じ理由である。

出品者が今もいるかに答えるのは§6.2で、**開いた1人についてだけ**である。検索結果全体へ
広げない理由は[TODO O-8](../planning/todo.md#o-8--sellerの活動で検索結果を絞るのを打ち切った理由2026-09-04)。

#### 状態Filter（2026-09-05決定）

**選んだ状態と、それより良い状態を残す。** 一致ではなく**上限**である。
...
| 項目 | 決めたこと |
|---|---|
| 軸 | [§5.6](#商品の状態2026-09-04決定)がCardへ出しているのと**同じ値** |
| 効き方 | 選んだ状態**以上**（＝より良いほう）を残す |
| 選択肢 | **その検索結果に実際にあった状態だけ。** 番号順に良いほうから並べる |
| `状態不明` | **落とさない。** 選択肢にも出さない（下記） |
| 追加Request | **0。** Frontendだけで閉じる |
| 出す画面 | **検索画面だけ。** Seller商品一覧に状態が無い（[§5.6](#商品の状態2026-09-04決定)） |

**画面に番号を出さない。** Mercariの番号は`1`が`新品、未使用`で`6`が`全体的に状態が悪い`、
つまり**数の大小と状態の良し悪しが逆**である。`4以上`と書けば画面の言葉が意味と反対になるので、
`以上`は表示名へ付ける。

**良し悪しの順序の出所は`itemConditions` master表の並び順と、表示名そのものである**
（[観測結果](../../poc/mercapi/condition-result.md)）。
**Mercariは「番号に大小の意味がある」とは言っていない。** だから表に無い番号を
順序へ入れることもしない。

#### `状態不明`を落とさない

**未知の番号と欠落は`状態不明`とし、既知の値へ寄せない**と[§5.6](#商品の状態2026-09-04決定)が
決めている。`3以上`で`状態不明`を消すと、**不明を「悪い」と判定したことになる。**
表示のときにしない推測を、Filterのときだけすることはできない。

選択肢にも出さない。**`状態不明`は状態ではない**ので、選ぶものが無い。

#### 選択肢は結果から作る

**表示名の表をFrontendへ複製しない。** 番号→表示名はBackendの`ITEM_CONDITIONS`が持ち、
Cardへ出ている名前もそこから来る。Frontendが2つ目の表を持つと、Mercariが改名したとき
**Cardの名前とFilterの選択肢が食い違う。**

結果から作ると、副作用として**取得できていない状態を選択肢に出さない。**
番号`6`は検索で一度も観測されていないので、通常は選択肢に現れない。

**次の検索結果にその状態が無ければ、選択は`すべて`へ戻す。** 残すと、選択肢に無い値が
選ばれたままになり、**画面は`すべて`と表示しながらFilterは効き続ける。** 他のFilterは
どの結果でも同じ意味を持つので、これはこのFilterだけの扱いである。

#### これも奥へ遡れるようにはならない

取得済みから取り除くだけである。状態をMercariへ送れば到達範囲が変わるが、**それはやらない**
（[TODO O-9](../planning/todo.md#o-9--状態をmercariへ送るのを今やらない理由2026-09-04)）。
**削った先が見えなくなる**取得側の操作と、いつでも戻せる表示側の操作は別物である。
...
| Filter | **この値で絞れる**（[§5.5](#状態filter2026-09-05決定)） |
...
- 最後に、取得できた商品1件の詳細を取得する（**+1 Request**。[非アクティブ](#非アクティブを出す2026-09-05決定)）
...
- **非アクティブ**（下記）。Mercariの`is_inactive`をそのまま出す。取得できなければ`-`表示
...
#### 「非アクティブ」を出す（2026-09-05決定）

**Mercariの答えをそのまま置く。** 商品詳細Responseの`seller`にある`is_inactive`を、
**Mercariの語のまま**「非アクティブ」として表示する。
...
**「最も新しい更新」とは別物である。** あちらは取得した商品からこちらが計算した推定、
こちらはMercari自身が持っている値。並べて置く。

##### 名前を選ばない

[Adapter仕様 §6.3](../phase-0/phase-0-f-adapter-spec.md#63-field対応表--どこから来て意味に根拠があるか)は
改名を**転記**と**主張**に分ける。`is_inactive` →「非アクティブ」は**転記**であり、
意味が動いていないため根拠を要さない。

**「休眠中」「退会済み」と書かない。** どちらも意味を選ぶ**主張**で、実測に支持されない
（[観測結果](../../poc/mercapi/inactive-result.md)。3回・標本139件）。とくに「退会済み」は
反証されている——`is_inactive`が`true`のSellerページは全件開けた。

##### 限界を画面に書く

**この値が何を指すかは確認できていない。** Mercariの画面に対応する表示が無く、
API＋DOMという手段では検証できない（`unverifiable`）。塞げないので隠さず書く。
...
商品が1件も取れなかったSellerや、追加Requestが失敗した場合は**「Mercariが何も言っていない」**
であって「Mercariが活動中と言った」ではない。**この1点が、転記を主張に変えてしまう唯一の経路である。**

##### 追加Requestは1件

商品1件の詳細を取る。`is_inactive`はProfile Responseに無く、商品詳細の`seller`にしか無いため、
Profileだけでは得られない。**どの商品でもよい**（同一Sellerの別商品で12 / 12一致）。
販売中の1件目を使い、**販売中が0件なら売却済みの1件目**にする。引退した出品者は販売中が
0件でありうるので、この落とし方が要る。両方0件なら取得しない。

**検索結果には出さない。** 1商品につき1 Requestが要り、1,000件で33分になる
（[O-8](../planning/todo.md#o-8--sellerの活動で検索結果を絞るのを打ち切った理由2026-09-04)）。
...
#### 安全停止だけが`Retry-After`を返す（2026-09-05決定）

**Backendが自分の判断で断っているのは安全停止だけで、いつまで断るかを言えるのもそれだけである。**
待ちが残っているあいだ、安全停止で終わった応答へ`Retry-After: <秒>`を付ける。
`partial=true`の200にも付ける。**結果が少し取れていても、次の1回まで待つことに変わりはない。**

- **Mercari由来の429には付けない。** 相手は「いつなら良い」と言っていない。
  こちらが数字を作ればそれは推測になる
- **待ちが終わったらHeaderごと出さない。** `Retry-After: 0`は「今すぐ、いつでも」と読め、
  送るたびに真で、決して偽にならない
- **`Access-Control-Expose-Headers`へ載せる。** Cross OriginではBrowserが
  Headerを画面へ渡さない。**Header自体は届いており、両側とも壊れて見えない**
- **日付形式は使わない。** 同じ機械の中の2つの時計を比べることになる

回復条件そのものは[§9](#9-ui状態とerror表示)、決めた経緯と60秒の出所は
...
| 安全停止 | 自動再試行せず、**あと何秒で再試行できるか**を表示。待ちが終わったら再実行Buttonを出す |
| 安全停止で**Mercariへ出ていない**とき | 上に加えて**「Mercariへは問い合わせていません」**を表示（[下記](#届かなかったことを画面が言う2026-09-05決定)） |

401 / 403 / 429 / Challengeで、Login情報入力やProxy変更を促す表示は行わない。

#### 再実行Buttonは、その画面が使った動詞で名乗る（2026-09-05決定）

**Buttonの文字は画面ごとに変える。** 共通Componentが決めると、押す前に何が起きるか分からない。

| 画面 | Label | その画面で押したButton・使った語 |
|---|---|---|
| 検索 | **`検索をやり直す`** | `検索`Button |
| Seller | **`取得をやり直す`** | 「Sellerの商品を取得中」 |

**`もう一度実行`をやめた。** 画面のどこにも「実行」という語が無く、**利用者が押したのは
`検索`Button**である。何を実行するのか読み取れない。

#### 届かなかったことを画面が言う（2026-09-05決定）

**安全停止で断られたRequestは、Mercariへ1本も出ていない。** そのことを画面へ書く。

| 状況 | 出るか | なぜ |
|---|---|---|
| 待ちの最中の取得 | **出す** | Backendが自分で答えている。**押してもMercariの負荷にならない**ことが、これ以外から読み取れない |
| 停止のきっかけになった3回目の拒否 | **出さない** | **これは実際にMercariへ出て拒否された。**出さないと嘘になる |

判定は`CollectionMeta`から行う。**取得できたPageが0で、記録されたErrorも0**なら、
外部Requestは1本も使われていない。Pageが返れば前者が増え、失敗すれば後者が増えるので、
Requestの使い道はこの2つしかない。

#### 安全停止からの回復は時間経過だけによる（2026-09-05決定）

**連続3回拒否で止まった状態は、60秒後に自動で解けるのではなく「あと1回だけ試してよい」に変わる。**
その1回を出すのは**利用者の操作**であり、画面は自分からRequestを送らない。成功すれば解除、
また拒否されればまた60秒待つ。

| 選ばなかった案 | 理由 |
|---|---|
| 解除しない（Process再起動のみ） | この表が既に「時間を置くよう表示」と言っている。**待っても直らないのに待てと言う**ことになる |
| 明示操作で解除するButton | 遮断を取り消すためだけのButtonは、見た瞬間に押される。安全停止の意味が残らない |

**60秒に測った根拠は無い。** 429もChallengeも実測できていないため、[§5.3](#53-収集範囲)の
2秒間隔と同じ性質の値である（安全側で、緩める理由が無い）。出所と経緯は
[TODO](../planning/todo.md#決めた--時間経過で解除2026-09-05)、
残り時間の伝え方は[§8](#安全停止だけがretry-afterを返す2026-09-05決定)。

**部分成功の再実行Buttonはそのまま残す。** この表の「部分成功」の行が定めているとおりで、
待ちの最中に押せば安全停止の表示と残り秒数が出る。押せてしまうことが分かることの方が、
Buttonが消えて理由が分からないことより良い。
...
- 安全停止のStatus規則のTest（**3 Request連続で拒否させてEndpoint経由で再現する**。回復と`Retry-After`も同じ場所で）
...
- **状態Filterが選んだ状態以上を残し、`状態不明`を落とさないTest**
- **選択肢が結果にある状態だけで作られ、次の検索に無ければ選択を手放すTest**
...
- [x] E2E受入Flowがすべて成功する
- [x] 商品検索とSeller分析で取得範囲・停止理由を常に確認できる
- [x] Mercari全体の古い順・指定期間の全件・Seller全商品であると誤認させる表示がない
- [x] Auctionを通常出品または確定価格と誤認させる表示がない
- [x] Seller Knowledgeがこの文書の同じ入力から決定的に同じ値を返す
- [x] 外部取得失敗を成功または0件として隠さない
- [x] 主要操作がKeyboardとMobile Layoutで利用できる
- [x] Playwright Fallback、Database、定期Crawl、Cacheが実装へ混入していない
- [x] 利用規約確認が必要な公開・商用・継続取得へ進んでいない
- [x] **人間が見る商品を効率的に絞れる** — 上の機械的な条件を全部満たしても、
  これが成り立たなければMVPは目的を果たしていない

**上の9件は2026-09-05に確認した。** 何をどう確かめたかは
[TODO](../planning/todo.md#phase-1をクローズした2026-09-05)に記録する。

**最後の1件だけは機械が判定できない。** 使った本人にしか決められない。
**2026-09-05に利用者が「効率的である」と答えた**ため`[x]`とした。
このときの答えと、まだ遅かったことは
[TODO](../planning/todo.md#phase-1をクローズした2026-09-05)に残す。

> **この問いはMVPで終わらない。** Phaseごとに同じ問いを立て、利用者が答える。
> 位置づけは[コンセプト §20](concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)。
