...
| # | 内容 | 理由 | 第2回での状況 |
|---|---|---|---|
| 1 | Runnerが商品詳細標本の販売形式内訳を記録していない | §5の偏りを導出ではなく記録から言えるようにする | **解消**（§12.3） |
| 2 | 未知形状・終了済みAuctionの実測標本が0件 | 実サービスで再現できない。L2のFixtureで担保する現状を継続する | **未観測のまま**（§12.5） |
| 3 | 通常出品の「検索 vs 商品詳細」一致がStep 1で測れていない | Step 2で10 / 10を確認しているが、経路が異なる | **解消**（§12.3） |

いずれも**本実行の合否には影響しない。** 1と3は次回L4までにRunnerを直す候補とする。

> **第2回で対応した。** 1と3はRunnerの標本選定を形式ごとの枠へ変えて解消した。
> 2は探し方を追加したうえで、なお標本を得られていない。§12.5に詳細を記す。
...
### 12.7 Auction価格と商品ページの照合（Step 2）
...
| 2 | 終了済みAuctionの標本が0件（`sold_out` 507件を判定した上で） | **未観測**として残す。§12.5 |
| 3 | 基準2は構造上100%にしかならず、基準1と情報が重複する | 回帰検知として残すが、L4固有の情報は持たない |
| 4 | 5試行は同一Keyword・同一時刻帯であり、時期をまたぐ安定性は測っていない | 2回の実行の差（§12.4）が部分的にその情報を与えている |
