...
### 率を記録するときの規約（2026-09-01追加）

L4第1回で、**全項目100%で合格しながら測定側に3つの弱点**が見つかった。原因はいずれも
記録の不足であり、実行時にはError一つ出ていない。同じ見落としを防ぐため次を規約とする。

| # | 規約 | これが無いと何が起きるか |
|---|---|---|
| 1 | 率には**標本の内訳**を併記する | `20 / 20（100%）`が、実は1種類だけの標本でも同じに見える |
| 2 | 「一致」「成功」の**判定規則**を数字の隣に書く | 包含判定と厳密比較が同じ「100%」として並ぶ |
| 3 | 取りうる**値の範囲**を確認する。2値しか出ないなら率として扱わない | 構造上100%にしかならない指標を、測定結果と誤読する |
| 4 | 確認できなかったものは**別に数える**。率へ混ぜない | 「確認できなかった」が「一致した」として集計される |
| 5 | **未観測**と**合格**を区別して書く | 標本0件が「問題なし」として通る |
| 6 | 集計値だけでなく**生の実測値**を`artifacts/`へ残す | 後から別の問いを立てたときに再実行が要る |

実例と経緯は[検証の落とし穴](../retrospectives/2026-09-01-verification-pitfalls.md)、
第1回と第2回の差分は
[ライブ受入検証結果 §12.1](../phase-0/phase-0-f-live-acceptance-result.md#121-何を変えどの弱点が解消したか)。
...
### Domain型のFieldを追加・改名するとき

外部の名前を別の名前へ写すとき、**それが転記か主張かを先に決める。**

- [ ] この改名は**転記**か**主張**かを判断した
- [ ] 主張なら、**なぜその意味だと言えるかを1行で書いた**
- [ ] 書けないなら、**元の名前に近い名前を使った**（意味を主張しない名前にする）
- [ ] 根拠区分（`observed` / `derived` / `assumed`）を決めた
- [ ] [Adapter仕様 §6.3](../phase-0/phase-0-f-adapter-spec.md#63-field対応表--どこから来て意味に根拠があるか)のField対応表へ追記した
- [ ] `assumed`の主張を画面へ出す計画になっていないことを確認した

`num_sell_items`（出品件数）を`total_sales_count`（累計販売件数）と名付けた誤りは、
このチェックのどれか1つで止められた。**値は正しく転記されており、Testもすべて緑だった。**
誤っていたのは名前が主張する意味だけで、それを検査する仕組みが無かった。
...
- [ ] **Tabでfocusを送ると、環が二重に見えること。** 塗り潰した`オークション`Badgeや
  Linkの上でも消えないこと（[§3.10](../product/design-tokens.md#310-品質下限26)）
