...
- [ ] **`rating`のスケールを確かめる（5段階か否か）** — [下記](#phase-1の前に潰すもの)。
  **未解決。** ただし2026-09-03に**塞がずに進める道**が見つかった（[次項](#評価は件数の内訳で出す2026-09-03決定)）。
...
- [x] Seller名表示
- [ ] **評価表示** — **件数の内訳（良い / 普通 / 悪い）で出す**（[上記](#評価は件数の内訳で出す2026-09-03決定)）。
  Domain・Adapter・Schemaに内訳のFieldを足す必要がある
- [ ] **出品者の最終活動を出す** — 取得した販売中商品の`updatedAt`の**最大値**。
  「最後に何かを動かした時刻」になる（[下記](#出品者がアクティブかを出す2026-09-03決定)）

#### 出品者がアクティブかを出す（2026-09-03決定）

**5年放置の出品を見つけても、出品者がMercariを辞めていたら買えない。**
実際に使ってみて出てきた指摘で、今の画面はここに何も答えていない。

**答えるデータは既に取れている。** Seller画面はその人の販売中商品を最大100件取るので、
その中の`updatedAt`の最大値が「最後に何かを動かした時刻」になる。
**新しいFieldも追加のRequestも要らない。**

| 商品 | 出品者 | 意味 |
|---|---|---|
| 放置 | **活動中** | **狙い目。**忘れられた出品で、しかも連絡がつく |
| 放置 | 不在 | 買えないかもしれない |
| 新しい | 活動中 | 普通の市場 |

**検索Cardには出せない。** 全出品者の分析が要り、1人あたり最大70秒かかる
（[MVP仕様 §5.6](../product/mvp-spec.md#56-商品card)が自動実行を禁じている理由と同じ）。
...
**2026-09-03時点で未解決。** 1-3は評価スコアを画面へ出さないことで先に進めた。
画面には「評価スコアは、尺度を確認できていないため表示していません。」と書いてある。
