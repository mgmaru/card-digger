...
> **2026-09-04に完了した。**
> 次は[`is_inactive`の実測](#出品者が生きているかを直接知る2026-09-04)で、**意味を確かめてから
> Seller画面へ出す**。その後に[1-0の残り2件](#残っているもの--未決)（安全停止の回復条件）を閉じ、
...
## 出品者が生きているかを直接知る（2026-09-04）

**Mercari自身が判定を持っていた。** 商品詳細Responseの`seller`objectに**`is_inactive`**という
真偽値がある。推定でも代理指標でもなく、Mercariの答えである。

2026-09-04の実測（使い捨てProbe。**再現するProbeは着手時に`poc/mercapi/`へ置く**）。

| 未更新日数 | `is_inactive` | 出品数 | 評価数 |
|---:|---|---:|---:|
| **197日** | **True** | 2 | 0 |
| 63日以下（25件） | すべてFalse | 14〜30,470 | 25〜29,369 |

**Trueだったのは、その検索でいちばん放置されていた出品1件だけ**である。探しているものと
方向は合っている。**ただし標本は1件しかない。**

### 先に意味を確かめる

`is_inactive`が何を指すのかを確認していない。**「休眠」なのか「退会」なのか、評価0の
新規利用者と区別できるのかが分からない。** 意味を確かめずに画面へ出せば、
[Field対応表](../phase-0/phase-0-f-adapter-spec.md#63-field対応表--どこから来て意味に根拠があるか)の
`assumed`をそのまま表示することになる（`num_sell_items`を「販売件数」と読んだのと同じ形。
[検証の落とし穴](../retrospectives/2026-09-01-verification-pitfalls.md)）。

- [ ] `is_inactive`が何を表すかを実測する（L4。20〜30 Request、1分程度）
- [ ] Seller画面へ出す（**追加1 Request**。下記）

### 着手前に決めること — **5件、未決**

**今回の測定は使い捨てのProbeで、残していない。** 実測をやり直すところから始める。
書き方の型は[`condition_probe.py`](../../poc/mercapi/condition_probe.py)と
[その結果文書](../../poc/mercapi/condition-result.md)がそのまま使える。

| # | 決めること | なぜ先に決めるか |
|---|---|---|
| 1 | **`True`の標本をどう集めるか** | 1,000件の検索で**1件しか出なかった**。放置出品が多い母集団でないと標本が増えない。価格帯を狭めて`reachedEnd`まで取る必要がある |
| 2 | **何を正として突き合わせるか** | 商品ページ・Sellerページに対応する表示があるかを**確認していない**。無ければ`unverifiable`で、その場合に画面へ出すかを別に決める |
| 3 | **新規利用者と休眠を区別できるか** | 唯一の`True`は**出品2件・評価0**だった。「新規だから`True`」なら意味が反転する |
| 4 | **どの商品から取るか** | Sellerの商品1件の詳細を取る。**販売中が0件のSeller**では売却済みから取るのか、取らないのか |
| 5 | **画面の語とDomainへの乗せ方** | `Seller`型に足すのか別に持つのか。[`assumed`のまま画面へ出さない](#phase-1の前に潰すもの)との整合 |

**1と3が本体である。** 標本が集まらなければ率を書けず、新規と休眠を分けられなければ
「非アクティブ」と読ませてはいけない。
...
| | 追加Request | 判断 |
|---|---:|---|
| **Seller画面** | **+1**（既に9使っている画面へ、商品1件の詳細を足す） | **やる価値がある** |
| 検索結果 | 1商品につき1。1,000件で33分 | **打ち切り**（[O-8](#o-8--sellerの活動で検索結果を絞るのを打ち切った理由2026-09-04)） |

Seller画面の「最も新しい更新」は*取得できた範囲での推定*だが、`is_inactive`は**Mercariの答え**である。
並べて置くことで、開いた1人についての確度が上がる。**検索結果を絞る機能ではない。**
...
**開いた1人について確かめる道は残っている。** Seller画面の「最も新しい更新」と、
[`is_inactive`](#出品者が生きているかを直接知る2026-09-04)である。
