...
> **Phase 1のSearch MVPは2026-09-05にクローズした**
> （[根拠](#phase-1をクローズした2026-09-05)）。
> [1-0](#1-0-application基盤)から[1-5](#phase-1-5--e2e受入flowとlayout確認)までの実装は
...
>
> **次は[Phase 2](#phase-2--画像解析)。**
> **やる順に、[まず測る2件](#まず測る2件)と
> [測ってから決める4件](#測ってから決める4件)**を2026-09-05に整理した。
> **測る2件のうち画像URLの枚数は2026-09-05に測り終えた**
> （[結果](../../poc/mercapi/photos-result.md)、[要点](#写真は解析するのではなく見せるだけでよい2026-09-05実測)）。
> **検索は1出品あたり1枚しか返さず、利用者が写真でやっているのは解析ではなく閲覧だった。**
> 残る1件（見送った理由を20〜30件記録する）は**利用者にしかできない。**
>
> 並行して、[価格を判断しない探索補助](#価格を判断しない探索補助--使ってみて追加したくなったら)から
> **使ってみて追加したくなったものを実装する。**
> **どちらも「使うこと」から材料が出る**ので、同じ作業の裏表になる。
> 1件目は[状態Filter](#使ってみて分かった1件2026-09-05)で、2026-09-05に実装した。
>
> 次の候補だったA（売却済み商品の価格分布）は、**2026-09-05に見送った。**
> [card-pulse](https://github.com/mgmaru/card-pulse)が「フリマサイトの成約価格」を
> 自分の担当に挙げており、**価格の判断はCard Diggerが持たない**と決めたため
> （[コンセプト §19](../product/concept.md#19-相場データはcard-diggerが持たない2026-09-05決定)、
> [経緯](#売却済み商品の価格分布は作らない--card-pulseの担当2026-09-05決定)）。
...
- [x] 安全停止のStatus規則のTest（**2026-09-05にEndpoint経由でも再現できるようにした**。[下記](#決めた--時間経過で解除2026-09-05)）
...
### 決めた — 時間経過で解除（2026-09-05）

**2026-09-02から3回後回しにしていた1件を閉じた。** 安全停止（Circuit Breaker）だけが
Request単位のままで、`stop_reason = safety_stop`はEndpoint経由で発生しなかった。

- [x] **安全停止からの回復条件を決めた** — **時間経過で解除。60秒後に1回だけ試せる**
- [x] **Circuit Breaker部分もProcessで共有した** — `SafetyBrake`として`MarketplaceAccess`が持つ

#### 「戻らない」と「到達しない」は同じ1つの誤りだった

未決の書き方は「共有したいが、戻らない状態なので共有できない」だった。**2つの問題に見えていたが、
原因は1つである。** 連続拒否数の置き場所を
[アーキテクチャ §2.2](../development/architecture.md#22-状態の寿命--置き場所は何についての事実かで決まる)は
「**その収集についての事実**（この実行の中で連続）」と書いていた。

**その読み方では3に到達できない。** 収集は最初の失敗でPage送りを止めるので、収集ごとに数えると
値は最大1にしかならない。数えたかったのは初めから「**Mercariがこちらを拒み続けているか**」であり、
それはMercariについての事実である。同じ表の1行目「最後にMercariを叩いた時刻」と同じ側にある。

置き場所を直すと、共有と到達が同時に直った。**§2.2の表を1行書き換えた**（この変更で直した）。

#### 3案を較べた

| 案 | 採否 | 理由 |
|---|---|---|
| **時間経過で解除** | **採用** | [MVP仕様 §9](../product/mvp-spec.md#9-ui状態とerror表示)が既に「時間を置くよう表示」と約束している。**仕様を書き換えるのではなく、後から真にする**案はこれだけだった |
| 解除しない | 不採用 | 共有した瞬間に害へ変わる。一過性の拒否3回で再起動するまで道具が死に、**回復手順がアプリの外（Ctrl-C）へ出る** |
| 明示操作で解除するButton | 不採用 | 遮断を取り消すためだけのButtonは、見た瞬間に押される。Frontendは既にこれを避けており（`SearchPage.tsx`）、覆すだけの新しい根拠が無かった |

**「時間が経ったら自動で再試行する」ではない。** 待ちが終わって変わるのは
「**次に利用者が求めた取得を断らなくなる**」ことだけで、背景からRequestは出さない。
これで §9 の「自動再試行せず」と「解除できる」が両立する。
**しかも試行の頻度は最大でも60秒に1回で、通常の2秒間隔より低い。** 回復させる方がMercariに対して
安全側になる、というのがこの案を選べた決め手である。

#### 60秒に測った根拠は無い

**429もChallengeも一度も観測できていない。** Fork PR #2で401/403/429がParse Errorへ潰れる問題は
直したが、実際に429を受けた記録は無く、`challenge`は[検出手段自体が無い](#残る既知の制約)。
Mercariが`Retry-After`を返すかも分からない。**数字を導ける材料がどこにも無い。**

60秒は2つを同時に満たす値として選んだ。**人が「止まった」と読める程度に長く、
1人用の道具が道具でなくならない程度に短い。** [2秒間隔](../development/architecture.md#41-外部アクセスの条件と2秒間隔の出所)と
同じ性質の値であり、同じ扱いをする。安全側で、緩める理由が無い。
**実測値を得た人が置き換え、出所を書く。**

#### `half-open`はFieldにしなかった

Circuit Breakerの3状態のうち`half-open`は、**連続拒否数を`3-1`に置いた状態**として表す。
もう1回拒否されれば3に届いて閉じ、成功すれば0に戻る。停止中フラグと半開フラグの2つを持つと、
**互いに合っていなければならない値を2つ持つ**ことになる。

#### 画面へは残り秒数を出す

**`Retry-After`で伝える。** 規則は
[MVP仕様 §8](../product/mvp-spec.md#安全停止だけがretry-afterを返す2026-09-05決定)。
Mercari由来の429には付けない。相手は「いつなら良い」と言っていないので、
こちらが数字を作れば推測になる。

**`Access-Control-Expose-Headers`へ載せる必要があった。** Cross OriginではBrowserが
Headerを画面へ渡さない。**Header自体は届いており、Backendから見てもFrontendから見ても
壊れて見えない。** 画面が黙って「時間を置いて」だけに戻る。Testを1件置いた。

画面は`FailureNotice`が持つ。両画面が同じ失敗表示を複製していたので、そのとき1つへまとめた。

#### 見つかった1件 — **検索の503が、全部「安全停止」に見えていた**

**私の変更で入ったものではなく、元からあった。** ③の「届かなかったと画面へ示す」を実装しようとして
気づいた。

Backendは失敗を**2つの形**で返す。

| Endpoint | 形 | なぜ |
|---|---|---|
| Seller Profileが読めない | `{"detail": {...}}` | 収集が無い。Requestを出す前に断られている |
| それ以外 | `{"items": [], "meta": {...}}` | **収集は起きて、短く終わった** |

Frontendは`detail`しか読んでいなかった。**429もChallengeも403も、検索から来た503は
すべて`safety_stop`に分類されていた。** 画面には
「続けて拒否されたため取得を止めました。時間を置いてからお試しください」と出る。
**1回429を受けただけで止まっていないのに、止まったと言っていた。**

`meta`も読むようにした。`stopReason`が`safety_stop`かどうかで分ける。
**Statusだけでは分けられない**（Rate Limitも安全停止も503）。

実画面で3状態を確認した。

| | 画面 | Button |
|---|---|---|
| Mercariのrate limit | `Mercariが一時的に応答を制限しています。時間を置いてください` | `検索をやり直す` |
| 安全停止の始まり（Mercariへ出た） | `…取得を止めました…` + `あと約60秒で再試行できます` | 無し |
| 待ちの最中（Mercariへ出ない） | 上に加えて **`Mercariへは問い合わせていません。`** | 無し |

#### Buttonの文字と、届かなかったことの表示

正本は[MVP仕様 §9](../product/mvp-spec.md#再実行buttonはその画面が使った動詞で名乗る2026-09-05決定)。

- **`もう一度実行`をやめた。** 画面のどこにも「実行」という語が無く、押したのは`検索`Buttonである。
  画面ごとに`検索をやり直す` / `取得をやり直す`と名乗る
...
実Browserで、停止後に実時間を空けてから`検索`Buttonを押して確かめた。

| | 停止からの経過 | `Retry-After` | 画面 |
|---|---:|---:|---|
| 停止直後 | 0.0秒 | 60 | あと約60秒 |
| 押さずに放置 | 10.0秒 | — | **あと約50秒**（押さなくても減る） |
| **`検索`を押す** | **30.3秒** | **30** | **あと約30秒**（60へ戻らない） |
| そのまま | 35.3秒 | — | あと約25秒 |
| **もう一度押す** | 35.6秒 | 25 | あと約25秒（連打しても基準がずれない） |
| 待ち明け直前 | 58.0秒 | — | あと約2秒／Button無し |
| 待ち明け後 | 62.0秒 | — | 残り秒数の表示が消え／**`検索をやり直す`が出る** |

**基準は「停止が始まった瞬間」で、押した瞬間ではない。** Backendの`SafetyBrake`が
停止開始時刻から`Retry-After`を計算し、Frontendは受け取った秒数から
**再試行できる時刻**を作って、そこへ向けて数える。

**Testで固定した2件。** どちらも壊して落ちることを確認している。

- 待ちの最中に押して30が返ったら、画面が30になる（画面まで通したTest）
- 新しい応答が来たら`FailureNotice`が基準を取り直す（Component単体）

#### 残しているもの — `検索`Button自体は待ちの最中も押せる

**待ちの最中、失敗表示のButtonは消えるが、Formの`検索`Buttonは押せるままにしてある。**
押してもMercariへは出ず、その時点の残り秒数が出る。

利用者から**主要な操作を取り上げない**方を採った。Keywordを直したくなることもあり、
押した結果が「あと約N秒」と分かる表示になっている以上、罰する必要が無い。
**逆にしたい場合は、Formが失敗の状態を知る必要がある。**

**残り秒数ではなく「再試行できる時刻」を持つ。** 最初は`retryAfterSeconds`をそのまま渡していたが、
**画面を離れて戻るとComponentが作り直され、待ちが60から数え直しになる。** 既に過ぎた60秒を
表示することになる。`ApiError`が受信時刻から`retryAllowedAt`を計算して持ち、画面はそこへ向けて
数える。`collectedAt`を持って経過を計算するのと同じ形である。

#### 実測（2026-09-05）

429を返し続けるMarketplaceでApplicationを組み立て、**実時計**で叩いた。
`Mercariへ`はそのRequestが外部Requestを1本出したかどうかで、Adapterの呼出回数で数えている。
5回目の外部Requestだけ成功を返すようにした。

| | 停止からの経過 | Status | `stopReason` | `Retry-After` | Mercariへ | Mercari累計 |
|---|---:|---:|---|---:|---|---:|
| 1回目 | — | 503 | `error` | 無し | 出た | 1 |
| 2回目 | — | 503 | `error` | 無し | 出た | 2 |
| **3回目** | **0.0秒** | 503 | **`safety_stop`** | **60** | 出た | 3 |
| 4回目（直後） | 0.0秒 | 503 | `safety_stop` | 60 | **出ない** | 3 |
| | 30.2秒 | 503 | `safety_stop` | **30** | **出ない** | 3 |
| | 59.1秒 | 503 | `safety_stop` | **1** | **出ない** | 3 |
| | **61.1秒** | 503 | `safety_stop` | **60** | **出た** | **4** |
| | 122.1秒 | **200** | `end_of_results` | 無し | 出た | 5 |

読み方は3つ。

- **`Retry-After`は実時間で減る。** 60 → 30 → 1。60秒は**3回目の拒否の瞬間**から数える
- **待っている間、Mercariへは1本も出ない。** 累計が3のまま動かない。
  **押しても外部へは出ないので、連打の代償はMercariに行かない**
- **61.1秒で1本だけ出た**（累計4）。それも拒否されたので**また60へ戻り**、
  次の61秒後（122.1秒）に出た1本が成功して**200へ戻った**

**`stop_reason = safety_stop`がEndpoint経由で初めて発生した。**

#### ライブ受入検証（L4）だけは解除しない

[L4の条件表](../phase-0/phase-0-f-live-acceptance.md#2-実施条件)が
「安全停止は以後の外部アクセスを停止」と書いており、**60秒で再開する実行はその条件表が
説明している実行ではなくなる。** `SafetyBrake(cooldown_seconds=NEVER)`を渡して、
L4だけ従来どおり戻らないようにした。
...
`rating`のスケール（5段階か否か）は**未解決のまま塞がずに進んだ**（[次項](#評価は件数の内訳で出す2026-09-03決定)）。
**画面が`rating`を出さない限り塞ぐ必要が無い**ため、2026-09-05に
[O-10](#オプション--判断済みで保留しているもの)へ移した。**Phase 1の未完了項目ではない。**
...
> **Aは2026-09-05に見送った。card-pulseの担当である**（[下記](#売却済み商品の価格分布は作らない--card-pulseの担当2026-09-05決定)）。
> 下の比較は当時の判断としてそのまま残す。**「Aを先にやる価値が高い」は、card-pulseを知る前の結論である。**
...
### 売却済み商品の価格分布は作らない — card-pulseの担当（2026-09-05決定）

**Aは実装しない。** [card-pulse](https://github.com/mgmaru/card-pulse)が扱うデータとして
**「フリマサイトの成約価格」を自分の担当に挙げている。** 役割分担の正本は
[コンセプト §19](../product/concept.md#19-相場データはcard-diggerが持たない2026-09-05決定)。
...
| | 問い | 母集団の単位 | [Phase 2](#phase-2--画像解析)が要るか |
|---|---|---|---|
| **A** | このまとめ売り、5,000円は高いか | **出品（束）** | **要らない** |
| **[Phase 3](#phase-3--相場分析)** | 推定総額・手数料・想定利益 | **カード1枚** | **要る**（写真から枚数と種類が分からないと始まらない） |

**Phase 3へ入れると、Phase 2の完了待ちになり、実態より遠くへ置くことになる。**

#### 見送った3つの理由

- **card-pulseが既に自分の担当だと書いている。** Card Digger側で作ると、
  同じ判断が2か所に生まれる
- **Aは弱い答えである。** 母集団が「同じ**検索語**」であって「同じ**商品**」ではない。
  `ポケカ 引退品`の成約価格には10枚の束も500枚の束も混ざるので、中央値が分かっても
  目の前の5,000円が高いかは決まらない。**枚数で正規化するにはPhase 2が要る**
- **入れる／消すのコストは対称ではない。** 1-2で未更新期間の棒を決めたときに書いた原則で、
  「今入れて後で消す」だけが仕様の往復ぶんだけ高くつく。**強い答えが来ることが決まっている以上、
  今のAが一番高い買い物になる**

#### 答えの無い問いが1つ残る

**card-pulse MVPの範囲は買取価格・1 TCG・2〜3店舗で、フリマの成約価格は入っていない。**
「向こうがやる」と書いて忘れないよう、[コンセプト §19](../product/concept.md#答えの無い問いが1つ残っている)へ
残していることを、ここからも指しておく。
...
# 価格を判断しない探索補助 — 使ってみて追加したくなったら

**上のとおり、価格の判断はCard Diggerが持たない。** 残りはどれも、取得済みの集合に対する
FilterかSortか、手元の記録である。

**性質が2つに割れる。**

| | Mercariへの追加Request | 永続化 |
|---|---|---|
| 除外キーワード / Seller Knowledgeで絞る / 専門性で並べる | **0** | **不要** |
| お気に入り / 確認済み / メモ / 検索条件保存 | 0 | **要る。**[1-0](#1-0-application基盤)の「DatabaseとUser認証を導入しない」を崩す判断になる |

**順序はこの並びではなく、使ってみた結果から決める**（この節の冒頭のとおり）。

追加Requestも永続化も要らないもの。

- [ ] 除外キーワード
- [ ] Seller Knowledgeによるフィルター
- [ ] TCG専門性が低いSellerを優先表示

**着手前に永続化の判断が要るもの。** 5件バラバラに決めず、まとめて1度で決める。
...
## 使ってみて分かった1件（2026-09-05）

**状態は出したが、状態で絞れなかった。** [2026-09-04の1件目](#1-商品の状態を検索cardへ出す)で
検索Cardへ状態を出し、そのまま使って出た不満である。**判断の軸としては足りているが、
件数を減らす手段になっていない。**

| 困っていること | やること | 追加Request |
|---|---|---|
| **状態の悪い商品を、1件ずつ読んで飛ばしている。** 軸はあるのに件数が減らない | 状態で取得後Filterする | **0。** Frontendだけで閉じる |

**性質は[更新日Filter](#2-更新日で取得後filterする)と同じで、[O-9](#o-9--状態をmercariへ送るのを今やらない理由2026-09-04)とは別である。**
混同しやすいので分けて書く。

| | [O-9](#o-9--状態をmercariへ送るのを今やらない理由2026-09-04)（**保留のまま**） | 状態Filter（**実装した**） |
|---|---|---|
| どこで絞るか | `itemConditionId`をMercariへ送る | **Frontendで、取得済みの集合から** |
| 到達範囲 | **変わる。**並べ替えとページングの前に効く | **変わらない** |
| 削った先 | **永久に見えない** | **見える。**選択を戻せば戻る |
| 消えた件数 | 分からない | **出ている。**`指定した条件に一致: X件 / Y件` |

**O-7が実装の前提条件に挙げた「何件が視界から消えたのかを画面に出せるか（出せないなら
入れない）」は、[§5.4](../product/mvp-spec.md#54-検索結果metadata)の取得範囲表示が既に満たしている。**
更新日Filterと同じ場所に出る。

- [x] 状態Filterを実装した（Frontendのみ。[MVP仕様 §5.5](../product/mvp-spec.md#状態filter2026-09-05決定)）

### 決めた2点

**どちらも表示のときに決めたことを、Filterでも崩さないためである。**

| 決めたこと | 理由 |
|---|---|
| **画面に番号を出さない** | Mercariの番号は`1`が最良で`6`が最悪。**数の大小と良し悪しが逆**なので、`4以上`と書くと画面の言葉が意味と反対になる。`以上`は表示名へ付ける |
| **`状態不明`を落とさない** | [§5.6](../product/mvp-spec.md#商品の状態2026-09-04決定)が「未知の番号と欠落は既知の値へ寄せない」と決めている。`3以上`で消すと**不明を「悪い」と判定したことになる** |

**選択肢は結果から作る。** 番号→表示名の表をFrontendへ複製すると、Mercariが改名したときに
**Cardの名前とFilterの選択肢が食い違う。** 副作用として、検索で一度も観測されていない
番号`6`（[観測結果](../../poc/mercapi/condition-result.md)）は通常、選択肢に現れない。

### これはPhase 2の「まず測る」の材料になる

**[地雷の正体を3つに分ける表](#まず測る2件)に、4つ目が要るかもしれない。**

| 地雷の正体 | 必要なもの |
|---|---|
| **もう画面に出ている構造化値で分かる** | **何も要らない。**Filterを1つ足すだけ |

**状態Filterが実際に使われた回数と、[見送った理由20〜30件](#まず測る2件)のうち
「状態が決め手だった」件数が、この行が要るかを決める。** 多いほどPhase 2の担当範囲は小さくなる。

**ただし、まだ何も測っていない。** 状態は**束1つに対して出品者が申告した値1個**であり、
数百枚の中身の地雷度と対応するかは未検証である。「中身」ではなく「申告の慎重さ」を
測っている可能性を、[`is_inactive`の見立てを外した2件](#実測で外していた見立て2件)と同じ
形で疑っておく。

**そして、Filterを入れた状態で記録すると、20〜30件の母集団が偏る。**
弾かれた商品には見送り理由が記録されない。**記録は`すべて`のまま取る。**
...
| O-10 | **`rating`のスケールを確かめる（5段階か否か）** | **画面が`rating`を出していない。**評価は件数の内訳（良い／普通／悪い）で出しており、スケールを取り違えても表示に影響しない | `rating`そのものを数値で画面へ出したくなったとき | **小。**ただし**確かめる手段が無い**。観測できた値は`5`だけで、母数となる尺度をMercariが公開していない |
...
- [ ] O-10 `rating`のスケールを確かめる（**1-3から2026-09-05に移した。画面が出さない限り不要**）
...
**2026-09-05に取得後の[状態Filter](#使ってみて分かった1件2026-09-05)を入れたが、O-9は保留のままである。**
Frontendで絞るのは取得済みの集合からで、**到達範囲は変わらず、選択を戻せば消えた商品も戻る。**
やる契機（「価格帯だけでは足りなくなったとき」）はまだ来ていない。
むしろ**この2つを混同しないことが、ここでいちばん壊れやすい約束である。**
...
## Phase 1をクローズした（2026-09-05）

**Phase 1に未完了の作業は無い。** 1-0〜1-5の実装は2026-09-03、使ってみて分かった2件は
2026-09-04、`is_inactive`と安全停止の回復条件は2026-09-05に閉じた。

[MVP完了条件](../product/mvp-spec.md#mvp完了条件)10件のうち、**機械が判定できる9件を確認した。**

| 条件 | 何で確かめたか |
|---|---|
| E2E受入Flowがすべて成功する | `npm run e2e`が6件成功（desktop 1280px / mobile 390pxの2 Projectで3件ずつ） |
| 取得範囲・停止理由を常に確認できる | 検索は`CollectionRecord`（`aria-label="取得範囲"`と`停止理由:`）、Sellerは`collectionRange.ts`の`rangeNote()`が状態ごとに出す |
| 誤認させる表示がない | `reached_end`は終端かつ上限で捨てたとき偽にする（[§9](../product/mvp-spec.md#終端と上限は同時に起こる2026-09-03)）。Unit Testで固定 |
| Auction誤認なし | `ItemCard`のTestが`通常出品` / `オークション` / `形式不明`の3つを固定。`unknown`を`fixed_price`へ畳まない |
| Seller Knowledgeが決定的 | `test_seller_knowledge.py`が38件 |
| 失敗を隠さない | 部分成功は200＋`partial=true`＋`errors`、0件はStatusで分類。API Testで固定 |
...
| Frontend（`npm run test`） | **223 passed / 0 failed**（10 files） |
| E2E受入Flow（`npm run e2e`） | **6 passed**（desktop / mobileで3件ずつ） |
| `npm run typecheck` / `npm run build` | 成功 |
| `python3 tools/check_docs_links.py` | 35 files、NG 0件 |
...
**「人間が見る商品を効率的に絞れる」は機械が判定できない。** Testも型検査もCIも触れられない。

**利用者の答えは「効率的である」。** これで10件すべてが埋まり、Phase 1を閉じた。

同時に、**この問いはMVPで終わらせない**と決まった。Phaseごとに同じ問いを立て、
利用者が答える。位置づけと理由は
[コンセプト §20](../product/concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)が正本。

**「はい」で終わらせず、まだ遅かったことを1行残す。** そこが次の作業の出所になる。
実際、この製品の追加作業はすべてその気づきから出てきた。

| 気づき | そこから出た作業 |
|---|---|
| 商品の状態が分からない / 更新日で絞れない | [使ってみて分かった2件](#使ってみて分かった2件2026-09-04)（2026-09-04） |
| 出品者が生きているか分からない | [`is_inactive`をSeller画面へ](#出品者が生きているかを直接知る2026-09-05に実装)（2026-09-05） |
| まとめ売りの値段が高いか安いか分からない | [A](#売却済み商品の価格分布は作らない--card-pulseの担当2026-09-05決定)（card-pulseの担当と決めた） |

**Phase 1では「まだ遅かったこと」を記録しなかった。** 答えが「効率的である」で、
そこから出す不便が挙がらなかったためである。**次のPhaseからは必ず1行残す。**
...
**[Phase 1は2026-09-05にクローズした](#phase-1をクローズした2026-09-05)。次はここになる。**

## 着手前にやること — **未決。まず測る（2026-09-05に整理した）**

**何も決めていない。決めるための材料が無い。**

**Phase 0が効いたのは、3方式を同じ条件で測ってから1つ選んだからである。** ここも同じ形にする。
**この節は起きる順に並べてある。**

```text
まず測る2件  →  測ってから決める4件  →  実装
```
...
> **実際のMercariの引退品の写真で測る。綺麗なスキャン画像で測らない。**

Card Diggerが相手にするのは**不特定多数の利用者が撮った写真**で、写りが良いとは限らない。
[検証の落とし穴](../retrospectives/2026-09-01-verification-pitfalls.md)は
「全項目100%で合格した後に6つの落とし穴が見つかった」記録である。**同じことが起きる。**

- [ ] **見送った理由を20〜30件記録する（利用者が行う）**

  **これは利用者にしかできない。**見送る判断は利用者のものだからである。
  [§20](../product/concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)の
  「まだ遅かったことを1行残す」の実践そのものにあたる。

  溜まると、地雷が3つのどれなのか分かる。

  | 地雷の正体 | 必要なもの |
  |---|---|
  | **文章に書いてある** | 説明文を取り込むだけ。**画像解析は不要** |
  | **写真を見ないと分からない** | ここで初めてPhase 2 |
  | **相場を知らないと分からない** | card-pulse（[§19](../product/concept.md#19-相場データはcard-diggerが持たない2026-09-05決定)で決めた担当） |

- [x] **検索1回で、1出品あたり何枚の画像URLが返るかを数えた（2026-09-05。[観測結果](../../poc/mercapi/photos-result.md)）**

  **答えは「先頭1枚だけ」。** `photos`も`thumbnails`も**120 / 120で1枚**、欠落0件。
  Adapterの転記は正しく、**転記する元が1枚しかない。**

  | 検索が返す枚数 | Phase 2に必要なRequest |
  |---|---|
  | 全部（最大20枚） | 画像のDLだけ。商品詳細は不要 |
  | **先頭1枚だけ ← これだった** | **商品詳細が1商品あたり1回要る** |

  **同じ実行で、この問いが「Request設計」から「利用者の不便の答え」へ変わった**
  （[下記](#写真は解析するのではなく見せるだけでよい2026-09-05実測)）。

### 写真は「解析する」のではなく「見せる」だけでよい（2026-09-05実測）

**[枚数の測定](../../poc/mercapi/photos-result.md)が、Phase 2の見立てを変えた。**
測る前に立てていた「残りの写真は既にブラウザまで届いていて捨てられている」は**外れていた。**

#### 利用者が写真でやっていることは、Phase 2の実装一覧と別物である

**見送り理由は現時点で1つで、「開いて、写真を見て、状態が悪いのでやめる」だった。**

| | 問い | 対応 | 要るもの |
|---|---|---|---|
| **利用者が写真でやっていること** | この束、傷んでいないか | **A（地雷を踏まない）** | **写真を出すだけ** |
| [実装一覧](#実装--測定と決定の後) | この束に何が入っているか | B（見逃さない） | OCR・カード同定 |

**[ゴールをAとBのどちらにするか](#1-ゴールがaかbかでphase-2は別のものになる)で
「Phase 2は手数を減らさない」と書いた矛盾の正体がこれである。**
Phase 2はBを作る計画で、**困っているのはAだった。**

#### 実測が決めたこと

| 分かったこと | 実測 |
|---|---|
| **一覧で全Cardに複数枚出すのは成立しない** | 120件ぶんの詳細＝**240秒**。検索1回20〜30秒の10倍 |
| **成立しないのは描画ではない** | 4枚は**1.2倍・+109ms**。「著しく遅い」に当たらない |
| **利用者が選んだ1件なら成立する** | 詳細1回**243ms**。今の「Mercariを開く」は**1,796ms**（下限） |
| **画像は状態を判断できる大きさ** | 幅1080px・JPEG・177KB。20 / 20デコード成功 |
| **3割は詳細でも1枚のまま** | 20件中6件。**Mercariを開いても写真が増えない** |

**依存も増えない。** Local modelも外部Vision APIも要らないので、
**下の4番はこの用途では起きない。**

#### まだ決めていない3件

- **写真を出す形**（Card内で展開・別画面・Overlay）。[視覚方針](../product/design-tokens.md)が持つ判断
- **3割の「1枚しかない」出品をどう見せるか。** 1 Request払って初めて「1枚だけ」と分かる
- **説明文を同じ1 Requestで取るか。** 詳細Responseは`description`も持っている。
  [地雷の分類表](#まず測る2件)の「文章に書いてある」を、追加Request0で試せる

---

### 商品1件の画面をつくる — やる順（2026-09-05に整理した）

**[上の実測](#写真は解析するのではなく見せるだけでよい2026-09-05実測)から出た作業。**
**設計を先に書き、実装しない。**新しい画面なので、`src/`より先にMVP仕様が要る。

#### 先に立てた原則

> **一覧はA（明らかな地雷を踏まない）、この画面はB（買うか決める）。**

**一覧が絞れていないと、この画面がいくら良くても
[§20](../product/concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)に負ける。**
50件開くなら、重い画面は毒である。2026-09-05の[状態Filter](#使ってみて分かった1件2026-09-05)が
一覧側なのは、この分担どおりである。
...
| やらない | 理由 |
|---|---|
| **一覧の全Cardへ複数枚出す** | 120件ぶんの詳細で**240秒**。検索1回20〜30秒の10倍（[実測](../../poc/mercapi/photos-result.md)） |
| **画像解析** | この用途では要らない。人が写真を見れば2秒で終わる |

#### 画面は4段で考える

| 段 | 出すもの | どこから | この着手に含めるか |
|---|---|---|---|
| **1** | **写真（全枚数）** | Mercari詳細 **1 Request** | **含める** |
| **2** | **説明文** | **同じ1 Request** | **含める。**追加Request **0** |
| 3 | 人が指定した1枚の相場 | card-pulse | **含めない。**区画と名前だけ決める |
| 4 | 束の推定総額 | [Phase 2実装一覧](#実装--測定と決定の後) + Phase 3 | 含めない |

**段3を「含めない」にする理由。** card-pulseは
[§19](../product/concept.md#19-相場データはcard-diggerが持たない2026-09-05決定)の時点で
**構想・調査文書だけの段階**で、API・スキーマ・どちらが呼ぶかが未定である。
**まだ無いものに合わせて作ると空振りする。**
...
**`get_item`は既に`MarketplacePort`にあり、`RequestGate`を通っている**
（`analyze_seller`が`Operation.ITEM`で使っている）。**足りないのは`description`と、画面へ渡す口だけ。**

- [ ] `MarketplaceItem`へ`description`を足す
- [ ] Adapterの詳細経路で`description`を転記する
- [ ] **検索とSeller商品一覧の経路では`description`が来ないことを型で言う**（`None`。`item_condition`と同じ扱い）
- [ ] 商品1件を返すAPIを足す
- [ ] `ItemResponse`へ`description`を足す
- [ ] Mock Adapterへ写真複数枚と説明文を入れる
- [ ] 受入Fixture（`scripts/acceptance_app.py`）へ同じものを入れる。**「写真1枚だけ」の行も置く**
- [ ] Unit Testを足す（転記、欠落、経路ごとの`None`）
- [ ] Contract Testを足す（`MarketplacePort`の全実装が同じ約束を守る）
- [ ] API Testを足す（成功・404・安全停止）

#### C. Frontend — 段1と段2

- [ ] Routeと画面の骨を作る
- [ ] 写真を全枚数見せる（見せ方はAで決めたもの）
- [ ] **「写真はこの1枚だけ」を出す**（開く価値が無いことが、これで分かる）
- [ ] 説明文を出す
- [ ] 取得中・失敗・安全停止の表示
- [ ] 検索結果へ戻ったとき**再取得しない**Test
- [ ] Component Testを足す
- [ ] Mobile Layoutを確認する（[Test運用規約 §12](../development/test-policy.md)の手順）

#### D. 使って測る

- [ ] **説明文に地雷が何割書かれているかを、使いながら数える（利用者）。**
      [地雷の分類表](#まず測る2件)の「文章に書いてある」が、追加Request0で試せる
- [ ] [見送った理由20〜30件](#まず測る2件)の記録を続ける（**まだ残っている1件**）

#### E. 相場 — card-pulse待ち

- [ ] card-pulseのAPI・スキーマが決まるのを待つ（[§19](../product/concept.md#連携の形はまだ決めていない)）
- [ ] **人が「写っているカード」を入力する形を決める。**
      同定を機械がやる必要は無く、[実装一覧](#実装--測定と決定の後)の
      「人間が修正できるUI」が主役になるのはここである
- [ ] 相場Portを実装する（Mockを先に）
- [ ] 画面へ段3の区画を足す
...
- [ ] **1. Phase 2のゴールをAとBのどちらにするか**
- [ ] **2. 同定結果を「確定」で出すか「候補＋確度」で出すか**
- [ ] **3. 画像取得を2秒間隔の対象にするか**
- [ ] **4. 解析をどこで走らせるか。`.env`を導入するか**

---

#### 1. ゴールがAかBかで、Phase 2は別のものになる

[MVP後 — 探索補助](#mvp後--探索補助)でAとBを分けたとき、こう書いた。

> Bは「**見逃さない**」ための機能、Aは「明らかな地雷を踏まない」ための機能で、
> **探索の手数を減らすのはAである。**

**このBがPhase 2 + Phase 3である。** つまり**我々自身が「Phase 2は手数を減らさない」と書いている。**

一方[コンセプト §20](../product/concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)は、
**「探索が速くなったか」をPhaseの合否条件にした。**

**このままだとPhase 2は自分の完了条件を満たせない。** Testが緑でも、
「効率的に絞れるようになったか」に「はい」と言えない。**ゴールを決め直すか、
合否条件が満たせないことを承知で進むかを、着手前に選ぶ。**

##### 決める前に、「明らかな地雷」が何かを言葉にする

**手段（画像解析）から目的を決めない。** 地雷の正体が分からないうちは、
それを検出するのに画像が要るかどうかも決められない。

**そして画像を使わない手札がまだ余っている。**

| 手札 | 状態 |
|---|---|
| **商品説明文** | **未取得。**Forkの`Item`は`description: str`を持つが、`MarketplaceItem`へ取り込んでいない |
| **写真の枚数** | **未使用。**`image_urls`の要素数であって、解析ではない |
| 価格・未更新期間・状態・Seller Knowledge・`is_inactive` | 実装済み |

**まとめ売りの地雷は、かなりの割合が文章に書いてある可能性がある。**
「ノーマル中心」「レアは抜いてあります」「プレイ用」「傷汚れあり」。
**そうであれば画像解析は1行も要らない。**

#### 2. Card Diggerが出すのは「確定」ではないかもしれない

[card-pulse](https://github.com/mgmaru/card-pulse)もカード同定をする。
**同じものを2か所で作る話ではない。目的が違い、そのため要求精度が違う。**

| | 何のための同定か | 要求精度 |
|---|---|---|
| **card-pulse** | 価格を紐づける | **確定。**型番・レアリティ・言語まで合っていないと値段が付かない |
| **Card Digger** | 開く価値があるか判断する | **候補で足りる可能性がある。**「高額そうなものが写っていそう」で用が足りる場面がある |

card-pulseは「OCRやカード同定の低信頼データは確定値に混ぜず、レビュー待ちに隔離する」と
書いている。**Card Diggerの出力がその口に合う形かどうかを決める。**
決めた結果は[コンセプト §19](../product/concept.md#19-相場データはcard-diggerが持たない2026-09-05決定)へ書く。

#### 3. 画像を2秒間隔の対象にするかは、まだ決めていない

[MVP仕様 §5.3](../product/mvp-spec.md#53-収集範囲)は「**すべての**Mercari Request」と書いているが、
**画像について文書はどこも答えていない。**

**現状：Backendは画像を1枚も取っていない。** 画面の画像はBrowserがMercariのCDNから
直接、**先頭1枚だけ**`loading="lazy"`で取っている。**Backendが画像を取りに行くのは
Phase 2が初めてになる。**

**「利用者が今見ている1枚」と「解析のために20枚まとめて落とす」は性質が違う。**
同じ規則で縛るかを決め、理由を[アーキテクチャ §4.1](../development/architecture.md#41-外部アクセスの条件と2秒間隔の出所)へ書く。

##### 一括解析は成立しない。1商品ずつになる

| | Request数 | 2秒間隔での所要 |
|---|---|---|
| 検索結果100件を一括 | 最大2,000 | **約70分。**検索1回20〜30秒の道具に載らない |
| **利用者が選んだ1商品** | **最大20** | **約40秒。**検索1回と同じ桁で、成立する |

**したがってPhase 2は「人が選んだ1商品だけ」の機能になる。**
実装項目の「Confidenceを表示する」「人間が修正できるUI」は**添え物ではなく主役**であり、
順番も変わる。

#### 4. 依存が桁で増える

Backendの依存は今3つ（`mercapi`・`fastapi`・`uvicorn`）である。

| 方式 | 代償 |
|---|---|
| Local model（OpenCV / ONNX / Tesseract等） | **依存が激増する。**精度は未知 |
| 外部のVision API | 依存は少ないが**APIキーが要る。**今`.env`は存在せず、[0-1](#0-1-開発環境)の「必須環境変数がないことを確認する」を崩す判断になる |

---

## 実装 — **測定と決定の後**

**下の並びは着手順ではない。** 上の4件が決まると、順番も粒度も変わる
（特に「Confidenceを表示する」「人間が修正できるUI」は主役になりうる）。

> **「商品画像を取得する」は[商品1件の画面](#商品1件の画面をつくる--やる順2026-09-05に整理した)のBが引き受ける。**
> 残りは**段4（束の推定総額）**にあたり、[利用者が写真でやっていること](#利用者が写真でやっていることはphase-2の実装一覧と別物である)とは
> 別の問いに答える。
...
**Phase 2を閉じるとき、[§20](../product/concept.md#20-効率的かは利用者にしか答えられない2026-09-05決定)の問いを立てる。**
...
[7] Mercari Adapter作成（完了: L4合格でPhase 0-F完了）
        ↓
[8] Search MVP（完了: 2026-09-05にPhase 1をクローズ）
        ↓
[9] 使う（現在）
     ├─ 追加したくなったものを実装する
     └─ 見送った理由を20〜30件記録する ──┐
...
**[9]に決まった順序は無い。** 実装の候補は
[価格を判断しない探索補助](#価格を判断しない探索補助--使ってみて追加したくなったら)にあり、
**使ってみた結果から選ぶ。**

**[11]へ進む条件は「測ったこと」である。**
[Phase 2の着手前にやること](#着手前にやること--未決まず測る2026-09-05に整理した)を参照する。
**測る前に方式を選ばない。**Phase 0が効いたのはそのためだった。
