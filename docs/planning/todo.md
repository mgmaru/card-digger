...
- [ ] 検索Cardへ状態を表示する（Adapterに対応表、`ItemResponse`へ追加、未知の番号は`状態不明`）
...
| Sellerの活動 | **その人**が最後に動いた時刻 | **無い。** 取得していない（[O-8](#o-8--sellerの活動で絞るのを今やらない理由2026-09-04)） |
...
- [ ] 更新日Filterを実装する（Frontendのみ。Backend変更なし）
...
- Seller画面へ「取得できた商品のうち最新の更新日時」を出しても足りなくなったとき。
  こちらは**追加Request 0**で、分析済みのSeller 1人についてだけ言う表示である
- Seller収集の費用が下がったとき
