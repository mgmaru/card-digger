...
| # | 問い | 現状 |
|---|---|---|
| a | `created`は**出品日時**か（別の時刻ではないか） | 未検証 |
| b | 出品者が**価格変更などの編集**をしたとき`created`は動くか | 未検証 |
| c | **再出品**で`created`はリセットされるか（商品IDも変わるか） | 未検証 |

**bとcが「動く」なら、「632日前」も`oldest`並びも成立しない。**

#### 手元のデータでは答えられない

- 構造サンプルは匿名化済みで**型しか持たない**（値が無い）
- L4のartifactsは`oldestCreatedAt` / `newestCreatedAt`だけで、商品ごとの`created`を保持していない
- `mercapi`は`created`と`updated`を別々に持つが、**両方の値を並べて観測したことがない**

#### 実験

- [ ] **実験A（即時・1〜数Request）** — 商品ページのDOMに出品日時に相当する要素があるか調べ、あればAPIの`created`と突き合わせる。**aが決着する**
- [ ] **実験B（1〜2日待ち・数Request）** — 商品を10件ほど記録して日をおいて再取得し、`updated`が変わったのに`created`が変わらない商品を探す。**bが決着する**
- [ ] **実験C（調査のみ）** — 再出品時の挙動をMercariの公式ガイドで確認する。実測より仕様確認が適切。**cが決着する**

> **実験Bは[終了済みAuctionの実験2](#l4ライブ受入検証)と同じ形の縦断観測である。**
> 対象商品を記録して1〜2日後に再取得する点が共通するため、**1回の実行にまとめられる。**

### `rating`（`star_rating_score`）

- [ ] スケールを確かめる（5段階か否か）。Seller画面へ評価を出す前
...
> 終了済みAuctionの調査（実験1・実験2）はここに置かない。あれは**既知の空白を埋める調査**であり、
> 判断済みの保留ではない。[0-F-5](#l4ライブ受入検証)に残している。
