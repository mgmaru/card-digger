...
> 次は[使ってみて分かった2件](#使ってみて分かった2件2026-09-04)を、商品の状態表示・更新日Filterの
> 順で入れる。その後に[1-0の残り2件](#残っているもの--未決)（安全停止の回復条件）を閉じ、
...
形は**「○日以上更新されていない商品だけ残す」の日数入力**とする。掲載日Filterが絶対日付なのは
`createdAt`が動かない値だからで、`updatedAt`は`collectedAt`からの経過で読む値である
...
- [x] 更新日Filterを実装した（Frontendのみ。[MVP仕様 §5.5](../product/mvp-spec.md#更新日filter2026-09-04決定)）
