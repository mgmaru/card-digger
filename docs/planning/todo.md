...
> **2026-09-04に完了した。**
> [`is_inactive`の実測](#出品者が生きているかを直接知る--打ち切った2026-09-04)も**2026-09-04に完了し、
> 打ち切った**（意味が確定せず、買い手に見える対応物も無い）。
...
## 出品者が生きているかを直接知る — 打ち切った（2026-09-04）

**`is_inactive`は使えない。** 商品詳細Responseの`seller`にある真偽値で、Mercari自身の判定では
あるが、**何についての判定なのかが確定しない。**
[観測結果](../../poc/mercapi/inactive-result.md)（3回、標本139件、True 17件）。

| 質問 | 結果 |
|---|---|
| Fieldはあるか | **ある。** 139 / 139。欠落0 |
| Trueの標本は集まるか | **集まる。** 価格帯を割って裾まで取れば17件 |
| **新規利用者と休眠を区別できるか** | **できない。** 登録日の範囲が両群で完全に重なる |
| 同一Sellerの別商品で一致するか | **一致する。12 / 12。** 出品者についての事実である |
| **買い手に見える対応物はあるか** | **無い。** Sellerページ・商品ページのどちらにも差が出ない |

**着手前の標本1件は偶然だった。** 「その検索でいちばん放置されていた出品」がTrueだったが、
実測では**最も放置された層（365日以上）のTrueは28件中3件**で、180〜364日の28件中9件より少ない。
**「放置されているほどTrue」ではない。**

言えるのは「**Trueは登録が比較的新しく規模の小さい口座に偏る**」までで、休眠と切り分けられない。
出品16件・評価22件の現役出品者を「非アクティブ」と表示しうる。

- [x] `is_inactive`が何を表すかを実測した（2026-09-04。L4を3回、API 273 Request・ページ60枚）
- [x] Seller画面へ出さないと決めた（下記）

### 着手前に決めること — **5件、決着した**

| # | 決めたこと |
|---|---|
| 1 | **価格帯を割って裾を取る。** 検索は`updated`降順で順序を変えられないため、母集団が尽きるまで狭めるのが唯一の道。**終端まで届いた帯だけが裾を見せる**（届いた帯は646〜2,706日、届かない帯は99〜435日） |
| 2 | **正が存在しない。** Sellerページ・商品ページを両群で開き、`data-testid`の語彙全体（25〜70個）と候補語11語で比べて差が出なかった。**`unverifiable`** |
| 3 | **区別できない。** True群の登録日626〜1,906日はFalse群の275〜4,493日に完全に含まれる。中央値の差はあるが線は引けない |
| 4 | **どの商品でもよい。** 同一Sellerの別商品で12 / 12一致した。**ただし使う場面が無くなった** |
| 5 | **Domain型へ載せない。** 下記 |
...
1. **意味が確定していない。** [`assumed`のまま画面へ出さない](#phase-1の前に潰すもの)に加えて、
   有用になる読み方（「この人は休眠している」）だけが実測に支持されていない
2. **Fork変更が要る。** `is_inactive`はForkのモデルに無い（`models/item/data.py`の`Seller`にも
   `mapping/definitions.py`にも無く、`region_code`と2つだけ落ちている）。
   出すには Fork変更 → 依存SHA更新 → Adapter・API・Frontend と、
   [O-3](#オプション--判断済みで保留しているもの)と同じ規模になる。
   **意味が確定していない値のために払う額ではない**

**再開する契機。** Mercariの画面かヘルプに`is_inactive`と対応する表示が現れたとき。
そのときは`inactive_probe.py`をそのまま実行すればよい（候補の要素名を`--`で渡す必要はなく、
両群のページ語彙を比べる形になっている）。

### 残ったもの — 未更新90日未満にTrueは1件も無かった
...
3回とも例外が無かった。**ただし意味は分からないままである。** 55件の未観測であって、
Mercariがそう決めていると確認したわけではない。**必要条件のように見えることを根拠にしない。**

**「開いた1人が生きているか」に答えるのは、引き続き
[Seller画面の「最も新しい更新」](../product/mvp-spec.md#最も新しい更新を出す2026-09-03決定)だけである。**
取得できた範囲での推定だが、**意味は確定している。**
...
`is_inactive`は実測して[打ち切った](#出品者が生きているかを直接知る--打ち切った2026-09-04)
（意味が確定せず、買い手に見える対応物も無い）。上の表の「1 Request・約21分」は、
**費用ではなく値の意味のほうで否決された。**
