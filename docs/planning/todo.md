...
- [ ] **予算を使い切れば放置出品に届くか、実測する。** 10ページで足りるかは未測定。
  足りなければ次のレバーは予算を増やすことだが、Mercariへのアクセスが増える判断になる
...
- [ ] **`rating`のスケールを確かめる（5段階か否か）** — [下記](#phase-1の前に潰すもの)。
  **未解決のまま。** 2026-09-03の実装では**評価スコアを画面へ出さないことで回避した**ので、
  1-3の他の項目は進められた。**この項目が塞いでいるのは評価スコアの表示だけである。**
