...
> **測るまで着手しない。**測る2件のうち1件（見送った理由を20〜30件記録する）は
> **利用者にしかできない。**
>
...
- [ ] **検索1回で、1出品あたり何枚の画像URLが返るかを数える**

  **Phase 2のRequest設計が、この1回で決まる。**

  Adapterは検索結果の`photos`をそのまま`image_urls`へ転記しているが、
  **1出品あたり何枚返るかを測っていない**（[Adapter仕様](../phase-0/phase-0-f-adapter-spec.md)は
  `observed`とだけ書いてあり、枚数の記録が無い）。

  | 検索が返す枚数 | Phase 2に必要なRequest |
  |---|---|
  | 全部（最大20枚） | **画像のDLだけ。商品詳細は不要** |
  | 先頭1枚だけ | 商品詳細が1商品あたり1回要る |
