...
> 現在は**Phase 1のSearch MVP**が次の目標。[1-0](#1-0-application基盤)の**実装は2026-09-02に完了**し
> （Backend、`api/`、Frontendの骨組み）、次は[1-V 視覚方針](#1-v-視覚方針)である。
...
- [ ] Mobile / Desktopの主要Flow確認。**画面が無いので1-1〜1-4の後**
  （tokenの見え方は390 / 768 / 1280 / 1440pxで確認済み）
...
- [ ] 掲載が古い順（`created_asc`。取得範囲内）
- [ ] 掲載が新しい順（`created_desc`）
- [ ] **更新が古い順（`updated_asc`）** — 長く触られていない出品。目的に最も近い
- [ ] 更新が新しい順（`updated_desc`）
- [ ] 価格の安い順
- [ ] 価格の高い順
- [ ] 取得ページ数・件数・最古・最新日時・取得時刻・打ち切り理由の表示
- [ ] 掲載日FilterがMercari全体を網羅しないことの表示
- [ ] **掲載日・経過日数の限界表示**（`created`が出品日時かは[検証手段が無い](#phase-1の前に潰すもの)。文面は[MVP仕様](../product/mvp-spec.md#掲載日と経過日数の限界を画面へ書く)にある）
...
Badgeの見え方とGrid列数は[1-V](#1-v-視覚方針)で決まった。**残る問いは次の1件だけ。**

- [ ] **掲載日と更新日時を、Cardに図として描くかどうか**

  §5.6は両方を**文字で**出すよう確定させている。1-Vの検討中に、それに加えて
  **全Cardで同じ時間軸を共有する細い図**（掲載日を点、更新日時から取得時刻までを線で描く）を
  試作した。**線が長いCardほど長く触られていない**ため、日付を1件ずつ読まずにGridを
  見わたして候補を拾える。

  [MVP仕様 §5.5](../product/mvp-spec.md#55-sortとfilter)が
  **`updated_asc`がProductの目的に最も近い**と書いているとおり、
  「触られていない期間」はこの製品が探しているものそのものである。
  Sortはその順に並べ替えるが、**どれだけ放置されているかの量は見えない。**

  **文字の表示を置き換えるものではなく、足すもの。**入れない判断も成り立つ
  （Cardの情報量が増える、§5.6に無い表現を足すことになる）。決めてから実装する。
...
- [ ] 出品日時表示（**商品ページには表示されない値**である旨を添える）
- [ ] 経過日数表示
- [ ] 更新からの経過時間表示（**商品ページと同じ値**である旨を添える）
- [ ] 元Mercariページへのリンク
- [ ] Seller分析画面へのリンク
- [ ] 画像取得失敗時のPlaceholder
- [ ] Responsive Grid
...
- [ ] **`rating`のスケールを確かめる（5段階か否か）** — [下記](#phase-1の前に潰すもの)。
  取りうる範囲を観測していないため、**評価を画面へ出す前**に必要
